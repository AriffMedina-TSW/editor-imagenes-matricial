...
// ============================================
// Nombre del estudiante: Ariff Iazid Medina Gómez
// Fecha: 18/11/2025
// Grupo: C
...
function convertirEscalaGrises(matriz) {
  ...
  // Convierte cada pixel a escala de grises
  for (let i = 0; i < dim.filas; i++) {
    for (let j = 0; j < dim.columnas; j++) {
      const p = matriz[i][j];
      const gris = Math.round(0.299 * p.r + 0.587 * p.g + 0.114 * p.b);

      nueva[i][j] = crearPixel(gris, gris, gris, p.a);
    }
  }
...
function mezclarImagenes(matriz1, matriz2, factor) {
  ...
  // Fórmula usada: valor = a*(1-factor) + b*factor
  for (let i = 0; i < dims1.filas; i++) {
    for (let j = 0; j < dims1.columnas; j++) {
      ...
      resultado[i][j] = crearPixel(
        ...
        limitarValorColor(a.a * (1 - factor) + b.a * factor)
      );
    }
  }
